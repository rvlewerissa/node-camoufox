...
export class CamoufoxServer extends EventEmitter {
    ...
    async start() {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                this.#process = spawn('python', ['-X', 'utf8', serverPath, '--config', `"${configArg}"`], {
                    cwd: __dirname,
                    env: process.env,
                ...
                timeoutId = setTimeout(() => {
                    if (!this.#wsEndpoint) {
                        this.#cleanup();
                        reject(new Error(`Server start timeout after ${this.#startTimeout}ms`));
                    }
                }, this.#startTimeout);
    ...
    async stop() {
        try {
            ...
            if (this.#browser) {
                ...
                await this.#browser.close().catch((e) => this.#debug(`Browser close error: ${e.message}`));
                this.#browser = null;
            }
            ...
            if (this.#process) {
                ...
                if (process.platform === 'win32') {
                    await new Promise((resolve) => {
                        const taskkill = spawn('taskkill', ['/pid', this.#process.pid, '/T', '/F']);
